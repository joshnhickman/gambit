...
data class ActorStunned(val action: Stun, val actor: Actor, val duration: Int) : Event("$actor stunned for $duration rounds")
data class ActorModified(val action: Modify, val actor: Actor, val change: Int) : Event("$actor ${action.effect.targetStat} modified by $change")
data class ActorDefended(val action: Defend, val actor: Actor, val amount: Int) : Event("$actor defended from $amount damage")
data class ActorAttacked(val action: Attack, val actor: Actor, val damage: Int) : Event("$actor attacks for $damage damage")
